...
const updateTransactionInSupabase = async (expense: Expense) => {
  try {
    const response = await fetch('/api/transactions', {
      method: 'PUT',
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
      console.error('Failed to update transaction in Supabase:', response.status, errorData);
      return null;
    }
...
function getExpenseTimestamp(expense: Pick<Expense, "createdAt" | "date">) {
  if (typeof expense.createdAt === "number" && Number.isFinite(expense.createdAt)) {
    return expense.createdAt;
  }

  if (expense.date) {
    const parsed = new Date(`${expense.date}T12:00:00`).getTime();
...
function extractChatAmount(input: string): number | null {
  const lower = input.toLowerCase().trim();
  const naturalMatch = lower.match(/(\d+(?:[\.,]\d+)?)\s*(milh(?:a|ã)o|milhoes|milhões|mil)\b/);

  if (naturalMatch) {
    const base = Number.parseFloat(naturalMatch[1].replace(".", "").replace(",", "."));
    if (Number.isNaN(base)) return null;

    const multiplier = naturalMatch[2].startsWith("milh") ? 1000000 : 1000;
    return base * multiplier;
  }

  const amountMatch = lower.match(/(\d+(?:\.\d{3})*(?:,\d+)?|\d+(?:,\d{3})*(?:\.\d+)?)/);
  if (!amountMatch) return null;

  const amountText = amountMatch[1];
  const hasComma = amountText.includes(",");
  const hasDot = amountText.includes(".");

  let normalized = amountText;
  if (hasComma && hasDot) {
    normalized = amountText.replace(/\./g, "").replace(",", ".");
  } else if (hasDot && /^\d{1,3}(\.\d{3})+$/.test(amountText)) {
    normalized = amountText.replace(/\./g, "");
  } else if (hasComma && /^\d{1,3}(,\d{3})+$/.test(amountText)) {
    normalized = amountText.replace(/,/g, "");
  } else if (hasComma) {
    normalized = amountText.replace(",", ".");
  }
...
function parseExpense(input: string): Expense | null {
  ...
  const amount = extractChatAmount(input);
  ...
  const descriptionMatch = lower.match(/(?:com|no|na|em|para)\s+(.+)/);
  const fallbackDescription = input
    .replace(/(\d+(?:[\.,]\d+)?)\s*(milh(?:a|ã)o|milhoes|milhões|mil)\b/gi, '')
    .replace(/(\d+(?:\.\d{3})*(?:,\d+)?|\d+(?:,\d{3})*(?:\.\d+)?)/, '')
    .replace(/\b(gastei|paguei|comprei|recebi|ganhei|entrada|saida)\b/gi, '')
    .trim();
  const description = descriptionMatch?.[1]?.trim() || fallbackDescription || input.trim();
...
function looksLikeTransactionMessage(input: string) {
  const lower = input.toLowerCase();
  const hasAmount = extractChatAmount(input) !== null;
  const hasTransactionVerb = TRANSACTION_VERBS.some((verb) => lower.includes(verb));
  const hasKnownCategoryHint = CATEGORY_KEYWORDS.some(([, keywords]) =>
    keywords.some((word) => lower.includes(word))
  );
  const hasAnalysisKeyword = ANALYSIS_KEYWORDS.some((keyword) => lower.includes(keyword));

  if (hasAmount && (hasTransactionVerb || hasKnownCategoryHint) && !hasAnalysisKeyword) {
    return true;
  }

  if (hasAmount && /^(r\$|\d)/i.test(input.trim()) && !hasAnalysisKeyword) {
    return true;
  }
...
function shouldUseGrokForMessage(input: string) {
  const lower = input.toLowerCase();
  const hasAnalysisKeyword = ANALYSIS_KEYWORDS.some((keyword) => lower.includes(keyword));
  return hasAnalysisKeyword && !looksLikeTransactionMessage(input);
}
...
function detectLeaks(expenses: Expense[]) {
  ...
  const recent = expenses.filter((entry) => now - getExpenseTimestamp(entry) <= windowDays);
...
export default function Home() {  const [messages, setMessages] = useState<Message[]>([]);
  ...
  useEffect(() => {
    if (modal?.type !== "edit-historico" || !modal.id) return;
      ...
      if (transactions.length > 0) {
        setExpenses(
          transactions.map((transaction: Expense) => ({
            ...transaction,
            createdAt: getExpenseTimestamp(transaction),
          }))
        );
      }
    };
  ...
  const dailySeries = useMemo(() => {
    ...
    expenses
      .filter((expense) => expense.kind === "expense")
      .forEach((expense) => {
        const date = new Date(getExpenseTimestamp(expense));
  ...
  const compass = useMemo(() => {
    ...
    const currentWindow = expenses.filter((entry) => now - getExpenseTimestamp(entry) <= sevenDays);
    const previousWindow = expenses.filter(
      (entry) => {
        const age = now - getExpenseTimestamp(entry);
    ...
    const spark = Array.from({ length: 8 }).map((_, idx) => {
      ...
      const value = expenses
        .filter((item) => {
          const timestamp = getExpenseTimestamp(item);
  ...
  async function askAIFallback(prompt: string) {
    ...
    if (!response.ok) {
      throw new Error(data?.text || "Prometheus AI esta indisponível no momento.");
    }

    return data?.text?.trim() || "Prometheus AI esta indisponível no momento.";
  }
  ...
  async function askAI(prompt: string) {
    ...
    try {
      ...
      if (!response.ok) {
        const fallbackText = await askAIFallback(contextualPrompt);
        updateMessage(messageId, fallbackText);
        return;
      }
      ...
      try {
        ...
        const message =
          fallbackError instanceof Error && fallbackError.message
            ...
            : "Prometheus AI esta indisponível no momento.";
  ...
  async function handleSend(text: string) {
    ...
    const parsed = looksLikeTransactionMessage(clean) ? parseExpense(clean) : null;
    if (!parsed && attachedFiles.length === 0 && shouldUseGrokForMessage(clean)) {
      askAI(clean);
      return;
    }
    ...
    if (!parsed) {
      pushMessage(
        "assistant",
        "Se for um lançamento, me diga no formato natural, como: paguei 120 no mercado. Se quiser análise, metas, saúde financeira ou busca de lançamentos, eu te ajudo também."
      );
      return;
    }
      ...
      <header className="header">
        <div className="brand">
          <div className="brand-mark brand-image" aria-hidden="true">
            <img src={BRAND_LOGO_SRC} alt={BRAND_NAME} />
          </div>
          <p className="brand-tagline">Seu copiloto para lancamentos financeiros com o poder da IA.</p>
        </div>
        <div className="chips">
          <button
            type="button"
                ...
                return (
                  <>
                    ...
                    <input
                      ...
                      onChange={(event) => {
                        const value = event.target.value;
                        setEditingHistory((prev) => ({ ...prev, description: value }));
                      }}
                    />
                    <input
                      className="input-mini"
                      ...
                      onChange={(event) => {
                        const value = event.target.value;
                        setEditingHistory((prev) => ({ ...prev, category: value }));
                      }}
                    />
                    <input
                      className="input-mini"
                      value={editingHistory.amount}
                      onChange={(event) => {
                        const value = event.target.value;
                        setEditingHistory((prev) => ({ ...prev, amount: value }));
                      }}
                    />
                    <select
                      className="input-mini"
                      value={editingHistory.kind}
                      onChange={(event) =>
                    ...
                    <div className="modal-actions">
                      ...
                      <button
                        ...
                        className="btn-primary"
                        onClick={async () => {
                          if (!editingHistory.description || !editingHistory.amount) return;
                          const updated = {
                            ...item,
                            description: editingHistory.description,
                            category: editingHistory.category || "Outros",
                            amount: Number(editingHistory.amount || 0),
                            kind: editingHistory.kind,
                            date: editingHistory.date || item.date,
                          };

                          const saved = await updateTransactionInSupabase(updated);
                          if (!saved) return;
